...
impl GarbageManager {
    ...
    pub fn cleanup_day(
        ...
        if day_dir.exists() && day_dir.read_dir()?.next().is_none() {
...
mod tests {
    ...
    fn test_scan_garbage_with_files() {
        ...
        assert_eq!(entries[0].logical_day.0, 738156);
        assert_eq!(entries[1].logical_day.0, 738156);
    }
    ...
    fn test_get_stats() {
        ...
        assert_eq!(stats.count_by_day[0].0 .0, 738156);
        assert_eq!(stats.count_by_day[0].1, 2);
        assert_eq!(stats.count_by_day[1].0 .0, 738157);
    ...
    fn test_cleanup_before() {
        ...
        // 清理 738155 之前的文件
        let deleted = manager
            .cleanup_before(logical_day::LogicalDay(738155))
            .unwrap();
    ...
    fn test_cleanup_day() {
        ...
        // 清理该日
        let deleted = manager
            .cleanup_day(logical_day::LogicalDay(738156))
            .unwrap();
    ...
    fn test_expired_size() {
        ...
        // 计算过期大小
        let size = manager
            .expired_size(5, logical_day::LogicalDay(738160))
            .unwrap();
        // 每个文件 12 字节
        assert_eq!(size, 24);
    }
